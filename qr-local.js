...
`
  ...
  const QRCodeModel = requireModule('index.js');
  const QRErrorCorrectLevel = requireModule('./QRErrorCorrectLevel');

  function createCanvas(text, options) {
    const { size = 250, colorDark = '#111111', colorLight = '#ffffff' } = options || {};
    ...
    const offset = Math.floor((size - canvasSize) / 2);
    ...
    for (let row = 0; row < moduleCount; row += 1) {
      for (let col = 0; col < moduleCount; col += 1) {
        if (qr.isDark(row, col)) {
          context.fillRect(offset + (col + quietZone) * scale, offset + (row + quietZone) * scale, scale, scale);
        }
      }
    }
  ...
  window.TagAlongQR = { createCanvas };
})();
